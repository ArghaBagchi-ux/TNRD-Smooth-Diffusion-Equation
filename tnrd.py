...
class MyModel:
    def __init__(self, in_channels):
        ...
        self.s1 = None  # Initialize s1 attribute
    ...
    def process_image(self):
        ...
        # Input tensor
        y = torch.randn(1, self.in_channels, 256, 256).cuda()  # Ensure it's on the GPU

        # Convert to numpy array for transformation
        y_np = y.cpu().numpy().squeeze(0)  # Move to CPU before converting to numpy

        # Apply Gaussian filter
        y1_np = gaussian_filter(y_np, sigma=sigma)

        # Convert back to tensor
        y1 = torch.from_numpy(y1_np).unsqueeze(0).cuda()  # Move back to GPU

        # Calculate s1
        M1 = torch.max(torch.abs(y1))
        nu = 1
        self.s1 = torch.pow(y1 / M1, nu)

        # Print or return s1 if needed for verification
        print(self.s1)
...
class TNRDConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size=5,
        ...
        # Initialize any parameters or layers needed
        self.in_channels = in_channels
    ...
    def forward(self, input,s1):
        ...
        # Perform another convolution with weight rotation
        ...
        # Update output based on s1
        if self.counter % 100 == 0:
            print(self.alpha, self.beta.max(), self.beta.min(), self.act.weight.max(), self.act.weight.min(), output.sum(1, keepdim=True).max())

        # Here we use s1 to modify the output
        output = u -(s1) * self.beta * output.sum(1, keepdim=True) - self.alpha * (u - f)
...
class TNRDlayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5,
        ...
        # Define your activation function and parameters
        self.act = RBF(_NRBF, self.in_channels, gaussian)
        self.alpha = nn.Parameter(torch.Tensor([0.9]))
        self.beta = nn.Parameter(torch.zeros([1, self.in_channels, 1, 1]))

        # Define your weights based on conditions like _DCT, _C1x1, _TIE
        if _DCT:
        ...
        # Initialize other required attributes or buffers
    ...
    def forward(self, input):
        ...
        for it in range(1):
            ...
            # Use s1 here to modify u or the computation
            

            # Perform convolution and other operations
            if _DCT:
                K = self.weight.matmul(self.dct_filters)
                K = K.div(torch.norm(K, dim=1, keepdim=True) + 2.2e-16).view(self.kernel_size ** 2 - 1, 1,
            ...
            if self.counter % 500 == 0:
                print(self.alpha, self.beta.max(), self.beta.min(), self.act.weight.max(), self.act.weight.min(),
                      output.sum(1, keepdim=True).max())
